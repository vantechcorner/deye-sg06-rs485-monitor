...
```text
  [ IRIV / ESPHome / ESP32 ]  = Modbus MASTER
         ...
         RS485 A/B @ 9600
              |
  [ Deye SG06  or  deye-sg06-ivt-emu ]  = Modbus SLAVE
```
...
| Area | Files |
|------|--------|
| **RS485 params / PDF** | `Deye SG05 Modbus Protocol.V118.pdf`, register notes in README |
| **IRIV IOC MQTT** | `iriv-ioc-config.json`, `_gen_iriv_jobs.py` |
| **Emulators (slave)** | `deye-sg06-ivt-emu.py`, optional `bms-pylon-emu.py` |
| **ESPHome** | `deye-sg06-nodemcu.yaml` |
| **Home Assistant** | `homeassistant/iriv_deye_mqtt.yaml` |
...
```bash
python _gen_iriv_jobs.py   # regenerates iriv-ioc-config.json
```
...
Flash `deye-sg06-nodemcu.yaml` (hostname `sg06-nodemcu`). Keep ~15 s poll + throttle to avoid `Frame already active`.
